CREATE TABLE ESTRATIFICACION_ODS (
  SUM_CON_ANO NUMBER,
  SUM_CON_PERIODO NUMBER,
  SUM_CON_SERVICIO NUMBER,
  IDENTIFICADOR_EMPRESA NUMBER,
  SUM_CON_DEPTO NUMBER,
  SUM_CON_MUNICIPIO NUMBER,
  SUM_CON_CNRPBLADO NUMBER,
  DANE NUMBER,
  SUM_CON_ESTRATO NUMBER,
  USUARIOS NUMBER,
  CODIGO_CENTRO_POBLADO NUMBER,
  CENTRO_POBLADO VARCHAR2(128 BYTE),
  LONGITUD VARCHAR2(38 BYTE),
  LATITUD VARCHAR2(38 BYTE),
  ALCALDIA_CONTEO NUMBER
);

---------------------------------------------------------------
-- ESTRATIFICACIÓN --- CONSULTA PARA OBTENER DATOS DE ESTRATIFICACION ---- CONSULTA QUE SIRVE PARA ACTUALIZAR LA TABLA DE DATOS DE ESTRATIFICACION
---------------------------------------------------------------
INSERT INTO ESTRATIFICACION_ODS(
SELECT TC1.*, DIVIPOLA.CODIGO_CENTRO_POBLADO, CONVERT(DIVIPOLA.NOMBRE_CENTRO_POBLADO, 'US7ASCII', 'EE8MSWIN1250') CENTRO_POBLADO, DIVIPOLA.LONGITUD, DIVIPOLA.LATITUD, ALCALDIA.ALCALDIA_CONTEO
FROM (--CONSOLIDADO COMERCIAL
SELECT C.SUM_CON_ANO, C.SUM_CON_PERIODO, C.SUM_CON_SERVICIO, C.IDENTIFICADOR_EMPRESA, C.SUM_CON_DEPTO, C.SUM_CON_MUNICIPIO, C.SUM_CON_CNRPBLADO, (LPAD(C.SUM_CON_DEPTO,2,'0') || LPAD(C.SUM_CON_MUNICIPIO,3,'0') || LPAD(C.SUM_CON_CNRPBLADO,3,'0')) DANE
...
WHERE C.SUM_CON_SERVICIO IN (4, 5)
    AND C.SUM_CON_ANO=:ANIO_ARG --ANIO (REMPLAZAR EL 2021 POR LA VARIABLE DEL FRONT)
    AND C.SUM_CON_PERIODO BETWEEN 1 AND 12  --MES (REMPLAZAR EL 1 POR LA VARIABLE DEL FRONT)
...
GROUP BY C.SUM_CON_ANO, C.SUM_CON_PERIODO, C.SUM_CON_SERVICIO, C.IDENTIFICADOR_EMPRESA, C.SUM_CON_DEPTO, C.SUM_CON_MUNICIPIO, C.SUM_CON_CNRPBLADO, (LPAD(C.SUM_CON_DEPTO,2,'0') || LPAD(C.SUM_CON_MUNICIPIO,3,'0') || LPAD(C.SUM_CON_CNRPBLADO,3,'0'))
...
LEFT JOIN (--ESTRATIFICACION Y COBERTURAS
            SELECT CAR_CARG_ANO, CAR_CARG_PERIODO, TO_NUMBER(LPAD(CAR_T1549_DANE_DEPTO,2,'0') || LPAD(CAR_T1549_DANE_MPIO,3,'0') || '000') DANE
                , ESTRATO, ALCALDIA_CONTEO FROM
                (
                SELECT CAR_CARG_ANO, CAR_CARG_PERIODO, CAR_T1549_DANE_DEPTO, CAR_T1549_DANE_MPIO, (CASE WHEN CAR_T1549_ESTRATO_ALCALDIA NOT BETWEEN 1 AND 6 THEN 9 ELSE CAR_T1549_ESTRATO_ALCALDIA END) ESTRATO, COUNT(CAR_T1549_PREDIAL_UTILIZADO) ALCALDIA_CONTEO
